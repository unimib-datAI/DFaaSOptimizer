...
\textsc{FaaS-MADiG}; the \texttt{capacity} criterion is the spare-capacity
analogue of the textbook shortest-queue power-of-$d$ rule (exact when sellers
are homogeneous in service rate). In the default
...
whole neighbourhood. Under the \texttt{capacity} criterion it is that
load-balancing rule verbatim; under the default \texttt{score} criterion it
keeps the random-$d$ sampling but selects by the model's economic score rather
than by load (see Differences~(i) below). It is, by design, \emph{less} novel
than \textsc{FaaS-MADiG} and is included as a controlled baseline, not as a new
algorithm.
...
criterion the buyer routes to the largest-spare-capacity (equivalently,
least-utilized) seller of the $d$ samples---the spare-capacity analogue of
Mitzenmacher's shortest-queue rule, exact when sellers are homogeneous in
service rate; with $d=2$ this is the canonical ``power of two choices''. Under
the default \texttt{score} criterion the same random-$d$ sampling is kept, but
selection maximizes the economic score rather than load (Differences~(i)). The
