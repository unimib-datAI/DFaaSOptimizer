...
For a buyer $i$, seller $j$, and function $f$, the implemented utility is
\begin{equation}
...
\State solve one local \textsc{LSP} for each node; obtain $x$, $r$, $\rho$, $U$, and initial $\omega^{0}$
\State $y\gets0$; $\omega\gets\omega^{0}$; $q\gets0$; $p\gets0$; replica-seller vector $\widehat\rho\gets0$
...
\Repeat
  ...
  \If{$\mathcal B\ne\emptyset$}
    ...
    \State $\omega_i^f\gets\omega_i^{0,f}-\widehat\omega_i^f$; set values within $\tau$ of zero to zero
  \EndIf
...
After all control periods, the runner joins the complete period solutions,
optionally produces history plots, and writes the final \textsc{LSP} and
\textsc{LSPc} solutions together with the objective, runtime, and termination
artifacts. These final writes are distinct from the conditional per-period
checkpoints in Algorithm~\ref{alg:magcaa}.
