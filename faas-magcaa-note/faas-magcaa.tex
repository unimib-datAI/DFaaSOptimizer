...
\Repeat
  ...
  \If{$\mathcal B\ne\emptyset$}
    ...
    \State reject the round if fixed $y$ fails the pre-solve \textsc{LSPr} memory-feasibility check
...
\Until{$\mathrm{stop}$}
\State decode the two tracked candidates; checkpoint and save \textsc{LSP}/\textsc{LSPc}, objective, termination, and runtime artifacts
\end{algorithmic}
\end{algorithm}
