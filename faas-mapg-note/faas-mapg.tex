...
\For{each node $i$ in order $\sigma$}
  ...
  \EndIf
  \State for every $f$ with $\max\{\omega_i'^f-\sum_j y_{ij}'^f,\ z_i'^f\}>\text{tol}$ and no inbound commitment, emit memory bids to neighbours $j$ with $\rho_j\ge\mathrm{RAM}^f$ and $\beta_{ij}^f>-\gamma_i^f$
\EndFor
...
memory-bid mechanism as \textsc{FaaS-MADeA}. Because the proposal MILP is
capped at the currently advertised residual capacity, unmet demand is read
from \emph{both} scarcity signals: the unplaced split residue
$\omega_i'^f-\sum_j y_{ij}'^f$ and the Cloud-forwarded residue $z_i^f$ of the
candidate state (load the node would rather offload than reject, since
$\delta_i^f>0>-\gamma_i^f$) — without the $z$-signal a saturated
neighbourhood would never grow and coordination would stall at the initial
residual capacity. For every such $f$ (excluding functions held as inbound
commitments, which the mover can never offload), node $i$ bids to every
neighbour $j$ with unused memory slack $\rho_j\ge\mathrm{RAM}^f$ and
