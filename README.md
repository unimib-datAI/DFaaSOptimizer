...
The network topology can be generated in three different ways, setting in the 
...
- a planar degree-3 (cubic) graph, by setting `"type": "planar"` and 
`"degree": 3`. The graph is generated using a circular ladder structure 
(`Nn` must be even and ≥ 6):
