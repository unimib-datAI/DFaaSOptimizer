...
Under which topology and connectivity conditions does hierarchical coordination
provide a measurable advantage?

**H4.** The advantage will be strongest in sparse, high-diameter topologies and
will shrink as graph density makes direct coordination sufficient.
...
scenario produces neither zero rejection nor universal overload. The calibrated
values will then be frozen for all confirmatory runs.
...
weighting forwarded traffic by edge latency. Fairness will be measured with
Jain's index over per-node served-load ratios. The analysis will also record the
maximum and 95th-percentile node-level rejection rate to expose tail behavior.
...
primary claims until that instrumentation is validated.
...
base scenario produces meaningful offloading and rejection.
...
### E3 — Topology and connectivity for RQ4
...
E3 contains 1,260 runs. Disconnected random graphs will be regenerated using the
same deterministic seed sequence, because connectivity must not be confounded
with topology family. The analysis will relate method performance to realized
mean degree, diameter, clustering coefficient, and average shortest-path length,
not only to the generator parameters.
...
nodes, 4 functions, the planar topology, 30 seeds, and the same six representative
methods used in E3, for 1,260 runs.
...
at 20 and 50 nodes on planar and random-regular degree-3 graphs using 30 seeds,
for 1,200 runs.
...
identified in E1 are tested on planar and random-regular degree-3 graphs with 50
nodes, 4 functions, and 30 seeds. This produces 1,680 runs.
...
ablation plot for E6, and a Pareto plot for E7. Tables should report experimental
factors, completion rates, effect estimates with confidence intervals, and the
final hyperparameters. Full per-cell summaries and termination conditions belong
...
Each batch definition must be immutable and serialized before execution. Every
result will be linked to the Git commit, configuration file, batch file,
manifest, method, instance seed, VM host, solver version, Python environment,
...
will consume only these versioned tables. Exclusions and reruns require a reason
code; failed runs must never be silently replaced.
...
Execution proceeds in the order E0, E1, E2, E3, E4–E5, E6, and E7. E0 must show
complete artifacts and centrally feasible outputs before E1 starts. E1 selects
the representative MABR method and the three compatible comparators used in later
families. E2 determines whether the 200-node cells are feasible within the
compute budget. E3–E5 establish external validity, while E6 and E7 explain the
...
The planned confirmatory workload is approximately 11,970 runs. After E0, the
...
insufficient, E7 becomes supplementary first; the primary E1–E3 comparisons and
...
study. The paper must report the conditions where hierarchy is neutral or
inferior, because identifying its operating envelope is part of the scientific
contribution.
